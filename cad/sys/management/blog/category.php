...
if ($_REQUEST['itemIDs'] != "") {
	$item_no = explode(",", $_REQUEST['itemIDs']);
...
if ($_POST['act2'] == "1") {
	...
	if ($_POST['type'] == "upd" or $_POST['type'] == "del") {
		...
		$_POST['meta_title'] = $arr_updl['meta_title'];
		...
		jQuery(function() {
			...
			jQuery('#submitSortable,#submitSortable2').click(function() {
				...
				if (confirm(itemNames + '【この順番でよろしいですか？】')) {
					location.href = '?itemIDs=' + itemIDs + '&t=<? echo time(); ?>&category_type=<? echo $_REQUEST['
					category_type ']; ?>';
				}
			});
		});
	...
	<div id="contents">
		...
		<form action="<?php echo $now_page; ?>?t=<?php echo time(); ?>" method="post">
			<div class="formbtn" style="margin-bottom:0px;">
				<span>
					<p class="or"><button type="button" id="submitSortable" name="add" style="width:180px;padding:5px;">この並び順を送信</button></p>
				</span>
			</div>
			...
			<div class="formbtn">
				<span>
					<p class="or"><button type="button" id="submitSortable2" name="add" style="width:180px;padding:5px;">この並び順を送信</button></p>
				</span>
		...
		<form action="<?php echo $now_page; ?>?t=<?php echo time(); ?>" method="post" name="mainform" enctype="multipart/form-data">
			<table class="Tblform">
				...
				<tr>
					<th>meta title</th>
					<td>
						<textarea name="meta_title" cols="60" rows="2" class="form_color_r"><?php echo $_POST['meta_title']; ?></textarea>
					</td>
				</tr>
				<tr>
			...
			<div class="formbtn">
				<span>
					<p class="gr"><button type="button" onClick="location.href='<?php echo $now_page; ?>?t=<?php echo time(); ?>&category_type=<?php echo $_POST['category_type'] ?>'" name="btn1">一覧ページにもどる</button></p>
				</span>
...
?>
	...
	<div id="contents">
		...
		<table class="Tblform">
			...
			<tr>
				<th>meta title</th>
				<td><?php echo NrDel($_POST['meta_title']); ?></td>
			</tr>
			<tr>
				<th>meta description</th>
...
if ($_POST['act'] == "end") {
	...
	if ($_POST['type'] == "regist") {
		...
		$stmt->bindParam(':meta_title', NrDel($_POST['meta_title']), PDO::PARAM_STR);
